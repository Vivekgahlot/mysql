...
select char_length( '     vivek    ');

select char_length (trim( '   vivek    '));

select char_length (rtrim( ' vivek     '));

select char_length (ltrim( '   vivek   '));
...
select cy.id, cy.name, cy.countrycode, cnt.code, cnt.name, cnt.continent from city as ct
join
country as cnt
